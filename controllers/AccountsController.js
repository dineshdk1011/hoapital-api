var AccountModel = require('../models/accountsLoginModel');
...
module.exports = {
    ...
    show: function (req, res) {
        ...
        AccountModel.findOne({ _id: id }, function (err, User) {
            if (err) {
                return res.status(500).json({
                    message: 'Error when getting User.',
                    error: err
    ...
    login: async function (req, res) {
        ...
        let user = await AccountModel.findOne({ email: req.body.email,role:req.body.role });
        if (user) {
    ...
    remove: function (req, res) {
        ...
        AccountModel.findByIdAndRemove(id, function (err, User) {
            if (err) {
                return res.status(500).json({

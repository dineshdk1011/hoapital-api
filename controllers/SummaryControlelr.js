...
module.exports = {
  ...
  list: function (req, res) {
    SummaryModel.find(function (err, Patients) {
      console.log(err);
      if (err) {
  ...
  show: function (req, res) {
    ...
    SummaryModel.find({ patientid: id }, function (err, Patient) {
      if (err) {
        return res.status(500).json({
          message: "Error when getting Patient.",
          error: err,
  ...
  create: function (req, res) {
    var Patient = new SummaryModel(req.body);
    console.log(Patient);
  ...
  update: function (req, res) {
    ...
    SummaryModel.findOne({ _id: id }, function (err, Patient) {
      if (err) {
        return res.status(500).json({
          message: "Error when getting Patient",
          error: err,
  ...
  remove: function (req, res) {
    ...
    SummaryModel.findByIdAndRemove(id, function (err, Patient) {
      if (err) {
        return res.status(500).json({

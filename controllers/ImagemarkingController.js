var GalleryModel = require('../models/Imagemarking');
...
module.exports = {
    ...
    show: function (req, res) {
        ...
        GalleryModel.find({ imageid: id }, function (err, Patient) {
            if (err) {
                return res.status(500).json({
                    message: 'Error when getting Patient.',
                    error: err
    ...
    remove: function (req, res) {
        ...
        GalleryModel.findByIdAndRemove(imageid, function (err, Patient) {
            if (err) {
                return res.status(500).json({
